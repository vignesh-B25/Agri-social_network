...
function pagination_link(){
    ...
    // Check if 'url' key exists in $_GET array
    $url = isset($_GET['url']) ? ROOT . $_GET['url'] : "";
...
function i_own_content($row){
    ...
    $myid = isset($_SESSION['mybook_userid']) ? $_SESSION['mybook_userid'] : null;
    
    if($myid === null || !is_array($row)) {
        return false;
    }
    ...
    //comments and posts
    if(isset($row['postid']) && isset($row['userid']) && isset($row['parent']) && isset($row['type'])){
